...
class ASCM(Algo):
    ...
    def intersecSet(self, set1, set2):
        '''
    ...
    def getSets(self, set1, set2):
        '''
            ...
            :return:
        '''
        intersectionSet = self.intersecSet(set1, set2)
    ...
    def step(self, x, last_b, history):
        ...
        if self.subsets_comb == 0:
            intersectionSet, sets = self.getSets(s_u, s_u)
        elif self.subsets_comb == 1:
            intersectionSet, sets = self.getSets(s_hv, s_u)
        elif self.subsets_comb == 2:
            intersectionSet, sets = self.getSets(s_mv, s_u)
        elif self.subsets_comb == 3:
            intersectionSet, sets = self.getSets(s_lv, s_u)
        elif self.subsets_comb == 4:
            intersectionSet, sets = self.getSets(s_ha, s_u)
        elif self.subsets_comb == 5:
            intersectionSet, sets = self.getSets(s_ma, s_u)
        elif self.subsets_comb == 6:
            intersectionSet, sets = self.getSets(s_la, s_u)

        elif self.subsets_comb == 7:
            intersectionSet, sets = self.getSets(s_hlv, s_u)
        elif self.subsets_comb == 8:
            intersectionSet, sets = self.getSets(s_hmv, s_u)
        elif self.subsets_comb == 9:
            intersectionSet, sets = self.getSets(s_mlv, s_u)
        elif self.subsets_comb == 10:
            intersectionSet, sets = self.getSets(s_hla, s_u)
        elif self.subsets_comb == 11:
            intersectionSet, sets = self.getSets(s_hma, s_u)
        elif self.subsets_comb == 12:
            intersectionSet, sets = self.getSets(s_mla, s_u)

        elif self.subsets_comb == 13:
            intersectionSet, sets = self.getSets(s_hlv, s_ha)
        elif self.subsets_comb == 14:
            intersectionSet, sets = self.getSets(s_hlv, s_ma)
        elif self.subsets_comb == 15:
            intersectionSet, sets = self.getSets(s_hlv, s_la)
        elif self.subsets_comb == 16:
            intersectionSet, sets = self.getSets(s_hmv, s_ha)
        elif self.subsets_comb == 17:
            intersectionSet, sets = self.getSets(s_hmv, s_ma)
        elif self.subsets_comb == 18:
            intersectionSet, sets = self.getSets(s_hmv, s_la)

        elif self.subsets_comb == 19:
            intersectionSet, sets = self.getSets(s_mlv, s_ha)
        elif self.subsets_comb == 20:
            intersectionSet, sets = self.getSets(s_mlv, s_ma)
        elif self.subsets_comb == 21:
            intersectionSet, sets = self.getSets(s_mlv, s_la)
        elif self.subsets_comb == 22:
            intersectionSet, sets = self.getSets(s_hla, s_hv)
        elif self.subsets_comb == 23:
            intersectionSet, sets = self.getSets(s_hla, s_mv)
        elif self.subsets_comb == 24:
            intersectionSet, sets = self.getSets(s_hla, s_lv)

        elif self.subsets_comb == 25:
            intersectionSet, sets = self.getSets(s_hma, s_hv)
        elif self.subsets_comb == 26:
            intersectionSet, sets = self.getSets(s_hma, s_mv)
        elif self.subsets_comb == 27:
            intersectionSet, sets = self.getSets(s_hma, s_lv)
        elif self.subsets_comb == 28:
            intersectionSet, sets = self.getSets(s_mla, s_hv)
        elif self.subsets_comb == 29:
            intersectionSet, sets = self.getSets(s_mla, s_mv)
        elif self.subsets_comb == 30:
            intersectionSet, sets = self.getSets(s_mla, s_lv)

        elif self.subsets_comb == 31:
            intersectionSet, sets = self.getSets(s_hlv, s_hla)
        elif self.subsets_comb == 32:
            intersectionSet, sets = self.getSets(s_hlv, s_hma)
        elif self.subsets_comb == 33:
            intersectionSet, sets = self.getSets(s_hlv, s_mla)
        elif self.subsets_comb == 34:
            intersectionSet, sets = self.getSets(s_hmv, s_hla)
        elif self.subsets_comb == 35:
            intersectionSet, sets = self.getSets(s_hmv, s_hma)
        elif self.subsets_comb == 36:
            intersectionSet, sets = self.getSets(s_hmv, s_mla)
        elif self.subsets_comb == 37:
            intersectionSet, sets = self.getSets(s_mlv, s_hla)
        elif self.subsets_comb == 38:
            intersectionSet, sets = self.getSets(s_mlv, s_hma)
        elif self.subsets_comb == 39:
            intersectionSet, sets = self.getSets(s_mlv, s_mla)
        elif self.subsets_comb == 40:
            intersectionSet, sets = self.getSets(s_mv, s_ma)

        elif self.subsets_comb == 41:
            intersectionSet, sets = self.getSets(s_hv, s_ha)
        elif self.subsets_comb == 42:
            intersectionSet, sets = self.getSets(s_hv, s_ma)
        elif self.subsets_comb == 43:
            intersectionSet, sets = self.getSets(s_hv, s_la)
        elif self.subsets_comb == 44:
            intersectionSet, sets = self.getSets(s_mv, s_ha)
        elif self.subsets_comb == 45:
            intersectionSet, sets = self.getSets(s_mv, s_la)
        elif self.subsets_comb == 46:
            intersectionSet, sets = self.getSets(s_lv, s_ha)
        elif self.subsets_comb == 47:
            intersectionSet, sets = self.getSets(s_lv, s_ma)
        elif self.subsets_comb == 48:
            intersectionSet, sets = self.getSets(s_lv, s_la)
